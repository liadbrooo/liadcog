...
class BaseSetupView(discord.ui.View):
    ...
    def _build_ui(self):
        ...
        self.admin_sel = discord.ui.Select(
            placeholder=self._role_placeholder(self.admin_role_id, "Admin-Rolle (für private Threads)"),
    ...
    async def _admin_select_cb(self, interaction: discord.Interaction):
        ...
        self.admin_sel.placeholder = self._role_placeholder(
            self.admin_role_id, "Admin-Rolle (für private Threads)"
        )
        await interaction.response.edit_message(view=self)
...
class SupportCog(commands.Cog):
        ...
        while not self.bot.is_closed():
            try:
                ...
                for guild_id in await self.config.all_guilds():
                    ...
                    if await guild_conf.last_daily_summary() != today:
                        await guild_conf.last_daily_summary.set(today)
                        await self.send_summary(guild, "daily")
                    if now.weekday() == 0 and await guild_conf.last_weekly_summary() != today:
            ...
            try:
                for guild_id, data in (await self.config.all_guilds()).items():
                    ...
                    for ticket in list(data.get("active_tickets", [])):
                        ...
                        if channel is None:
                            # Kanal weg -> Konfiguration sofort bereinigen
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            if ticket.get("status", "ACTIVE") == "PAUSED":
                ...
                is_allowed = message.author.guild_permissions.manage_messages or (
                    staff_role_id and any(r.id == staff_role_id for r in message.author.roles)
                )
                if not is_allowed:
                    try:
    ...
    async def ticket_set_wizard(self, ctx: commands.Context):
        ...
        while True:
            answer = await ask(
                "Bitte gib die **Admin-Rolle** an, die private Threads sehen soll "
                "(ID/Name – oder 'skip' für keine):"
            )
            if answer is None:
                return
    ...
    async def ticket_syncroles(self, ctx: commands.Context):
        ...
        for channel_id in thread_ids:
            ...
            try:
                ...
                log.exception("Sync fehlgeschlagen für Kanal %s", channel_id)
                skipped += 1
        await status_message.edit(
            content=(
                f"✅ Rollen in **{synced}** Thread(s) synchronisiert. "
                f"{skipped} übersprungen (gelöscht/nicht im Cache – archivierte Threads "
                "müssen ggf. einmal geöffnet werden)."
            )
        )
        ...
        try:
            if isinstance(channel, discord.Thread):
                if conf.get("delete_threads_after_close", False):
                    ...
                    await self._sync_roles_to_thread(channel, guild)
                    await channel.edit(
                        name=f"archiv-{channel.name}"[:100], archived=True, locked=True
                    )
            else:
                await channel.delete()
        except Exception:
    ...
    async def _add_role_members_to_thread(self, thread: discord.Thread, roles):
        ...
        if members:
            await asyncio.gather(*(_add(member) for member in members))
    ...
    async def _sync_roles_to_thread(self, thread: discord.Thread, guild: discord.Guild, cat_data: dict | None = None):
        ...
        await self._add_role_members_to_thread(thread, roles)
