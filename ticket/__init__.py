"""
SupportCog V32 - Final Interactive & Stable
- Interaktive Setup-Views (addcat und setup) funktionieren zuverlässig
- Views werden einmalig erstellt, nur Labels/Platzhalter werden aktualisiert
...
class BaseSetupView(discord.ui.View):
    ...
    def _build_ui(self):
        ...
        # Buttons
        self.btn_dm = discord.ui.Button(label=f"DMs: AN", style=discord.ButtonStyle.success, emoji='✉️', row=1)
        ...
        self.btn_del_thread = discord.ui.Button(label=f"Threads löschen: AUS", style=discord.ButtonStyle.danger, emoji='🗑️', row=2)
        ...
        self.btn_catstats = discord.ui.Button(label=f"Kategorie-Statistiken: AN", style=discord.ButtonStyle.success, emoji='📊', row=2)
        self.btn_catstats.callback = self._catstats_toggle_cb
        self.add_item(self.btn_catstats)

        self.btn_emoji = discord.ui.Button(label=f"Emoji-Balken: AN", style=discord.ButtonStyle.success, emoji='📈', row=2)
...
class CategorySetupView(discord.ui.View):
    ...
    def _build_ui(self):
        ...
        high_options = staff_options.copy()  # gleiche Rollenliste
    ...
    async def ticket_help(self, ctx: commands.Context):
        ...
        embed.add_field(
            ...
            value=(
                ...
                "2. `[p]ticket addcat` – Support-Kategorie interaktiv erstellen (Dropdown-Menüs).\n"
                ...
                "6. `[p]ticket stats [Kategorie]` – Statistiken.\n"
                "7. `[p]ticket export` – CSV-Export."
            ),
            inline=False
        )
        embed.add_field(
            name="👤 Für Nutzer",
            value=(
                "1. Im Ticket-Panel Kategorie auswählen.\n"
                "2. Anliegen beschreiben.\n"
                "3. Support-Team antwortet im Ticket-Channel/Thread.\n"
                "4. Nach Schließung Bewertung abgeben und Transkript erhalten."
            ),
            inline=False
        )
        embed.add_field(
            ...
            value=(
                ...
                "**Buttons:** ✋ Übernehmen, ⚠️ Eskalieren, 🔒 Schließen, Status-Dropdown."
            ),
            inline=False
        )
        embed.add_field(
            ...
            value=(
                "🟢 Aktiv – Auto-Close läuft.\n"
                "🟡 Wartet auf User – Auto-Close läuft.\n"
                "🔴 Wartet auf Team – Auto-Close pausiert, Auto-Eskalation möglich.\n"
                "⏸️ Pausiert – Ticket pausiert."
            ),
            inline=False
        )
        embed.add_field(
            ...
            value=(
                "Pro Kategorie kann eine maximale Anzahl aktiver Tickets festgelegt werden.\n"
                "Im Panel wird Auslastung angezeigt (z.B. `Support (3/10)`).\n"
                "Status `Wartet auf Team` oder `Pausiert` zählt nicht zur Auslastung."
            ),
            inline=False
        )
        embed.add_field(
            ...
            value=(
                "`[p]ticket stats` – Gesamtübersicht, Leaderboard, Kategorie-Statistiken.\n"
                "`[p]ticket stats Kategoriename` – Detailansicht einer Kategorie.\n"
                "`[p]ticket export` – CSV-Datei aller Statistiken."
            ),
            inline=False
        )
