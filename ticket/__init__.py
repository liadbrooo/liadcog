"""
SupportCog V19 - High-End Ticket System für RedBot
Fixes: Disable Control View on close (prevents double click bug), safe config updates (prevents panel deletion on temporary errors), removed asyncio.create_task leak.
"""
...
class CloseTicketModal(discord.ui.Modal, title='🔒 Ticket schließen'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # Defer sofort, um "Interaction Failed" zu verhindern
        await interaction.response.defer(ephemeral=True)
        await self.cog.close_ticket(interaction.channel, self.reason.value or "Kein Grund angegeben", interaction.user, interaction=interaction)
...
class BaseSetupView(discord.ui.View):
    ...
    def update_ui(self):
        ...
        log_sel = discord.ui.ChannelSelect(placeholder=log_ph, channel_types=[discord.ChannelType.text], custom_id="base_log", row=0)
        ...
        btn_dm = discord.ui.Button(label=f"DMs: {'AN' if self.dm_notifications else 'AUS'}", style=discord.ButtonStyle.success if self.dm_notifications else discord.ButtonStyle.danger, emoji='✉️', row=1)
        ...
        btn_auto = discord.ui.Button(label=f"Auto-Close: {self.autoclose_hours}h", style=discord.ButtonStyle.secondary, emoji='⏳', row=1)
        ...
        btn_cool = discord.ui.Button(label=f"Cooldown: {self.cooldown_minutes}m", style=discord.ButtonStyle.secondary, emoji='❄️', row=1)
        ...
        btn_max = discord.ui.Button(label=f"Max Tickets: {self.max_tickets_per_user}", style=discord.ButtonStyle.secondary, emoji='🔢', row=1)
        ...
        btn_finish = discord.ui.Button(label="Setup abschließen", style=discord.ButtonStyle.success, emoji='✅', row=2)
...
class CategorySetupView(discord.ui.View):
    ...
    def update_ui(self):
        ...
        btn_name = discord.ui.Button(label=f"Name" if not self.name else f"Name ✅", style=discord.ButtonStyle.primary, custom_id="cat_name", row=0)
        ...
        btn_desc = discord.ui.Button(label="Beschreibung" if not self.description else "Besch. ✅", style=discord.ButtonStyle.secondary, custom_id="cat_desc", row=0)
        ...
        btn_abbr = discord.ui.Button(label=f"Abkürzung" if not self.abbr else f"Abbr: {self.abbr}", style=discord.ButtonStyle.secondary, custom_id="cat_abbr", row=0)
        ...
        btn_emoji = discord.ui.Button(label=f"Emoji: {self.emoji}", style=discord.ButtonStyle.secondary, custom_id="cat_emoji", row=0)
        ...
        btn_save = discord.ui.Button(label="Save" if not self.cat_id else "Update", style=discord.ButtonStyle.success, emoji='✅', row=0)
        async def save_cb(inter: discord.Interaction):
            if not all([self.name, self.abbr, self.staff_role_id]): return await inter.response.send_message("Bitte fülle Name, Abkürzung und Support-Rolle aus!", ephemeral=True)
            if not self.discord_category_id and not self.thread_parent_id: return await inter.response.send_message("Bitte wähle entweder eine Discord Kategorie ODER einen Thread-Channel aus!", ephemeral=True)
...
class SupportCog(commands.Cog):
    ...
    async def send_dm(self, user: discord.User, title: str, description: str):
        try:
            embed = discord.Embed(title=title, description=description, color=discord.Color.blurple(), timestamp=datetime.datetime.now())
            await user.send(embed=embed)
        except: pass # Verhindert Crash, wenn User DMs gesperrt hat
                ...
                if t["channel_id"] == message.channel.id:
                    if t.get("status") == "PAUSED" and not message.author.guild_permissions.manage_messages:
                        try:
                            ...
                            await message.channel.send(f"{message.author.mention}, dieses Ticket ist pausiert. Du kannst aktuell nicht schreiben.", delete_after=5)
                        except: pass
                        return
    ...
    async def ticket_setup(self, ctx: commands.Context):
        ...
        msg = await ctx.send(embed=discord.Embed(title="🛠️ Ticket Basis-Setup", description="Konfiguriere Log-Channel, DMs, Auto-Close, Cooldown & Max Tickets.", color=discord.Color.blurple()), view=view)
        view.message = msg

    @ticket_cmd.command(name="addcat")
    async def ticket_addcat(self, ctx: commands.Context):
        view = CategorySetupView(self, ctx)
        msg = await ctx.send(embed=discord.Embed(title="🏷️ Kategorie Setup", description="Konfiguriere alle Werte. Wähle Channel ODER Thread.", color=discord.Color.green()), view=view)
        view.message = msg
        ...
        async def select_cb(inter: discord.Interaction):
            ...
            async def edit_cb(inter2: discord.Interaction):
                try:
                    ...
                    if not inter2.response.is_done(): await inter2.response.send_message(f"Error: {e}", ephemeral=True)
                    else: await inter2.followup.send(f"Error: {e}", ephemeral=True)
    ...
    async def update_panels(self, guild: discord.Guild):
        ...
        for p in panels:
            ...
            if not ch: 
                # Channel existiert nicht mehr, Panel verwerfen
                continue
            try:
                ...
                # Nachricht wurde gelöscht, Panel verwerfen
                pass
            except Exception:
                # Temporärer Fehler (z.B. Rechte fehlen kurzzeitig), Panel BEHALTEN!
                valid_panels.append(p) 
    ...
    async def add_role_to_thread_silently(self, thread: discord.Thread, role: discord.Role):
        # Kein asyncio.create_task mehr, direkt awaited, um Leaks zu vermeiden
    ...
    async def create_ticket(self, interaction: discord.Interaction, cat_id: str, issue: str):
        ...
        if cat_data.get("thread_parent_id"):
            parent_ch = guild.get_channel(cat_data.get("thread_parent_id"))
            if not parent_ch: return await interaction.followup.send("❌ Fehler: Der Thread-Channel wurde gelöscht. Admin muss `[p]ticket managecats` nutzen.", ephemeral=True)
            try:
                ...
                # Wird nun direkt awaited, kein Hintergrundtask mehr
                if staff_role: await self.add_role_to_thread_silently(ticket_channel, staff_role)
                if high_role: await self.add_role_to_thread_silently(ticket_channel, high_role)
            ...
            category = guild.get_channel(cat_data.get("discord_category_id"))
            ...
            return await interaction.followup.send("❌ Diese Kategorie hat weder einen Thread-Channel noch eine Discord-Kategorie. Admin muss `[p]ticket managecats` nutzen.", ephemeral=True)
        ...
        if config["dm_notifications"]:
            await self.send_dm(user, "🎫 Ticket erstellt", f"Dein Ticket auf **{guild.name}** wurde erfolgreich eröffnet.\n**Kategorie:** {cat_data['name']}\nEin Teammitglied wird sich bald um dein Anliegen kümmern.")
        await self.send_log(guild, "✅ Ticket eröffnet", discord.Color.green(), [("Ersteller", f"{user.mention} (`{user.id}`)"), ("Kategorie", cat_data['name']), ("Channel", ticket_channel.mention)])
    ...
    async def claim_ticket(self, interaction: discord.Interaction, view: TicketControlView):
        ...
        if ticket_data["claimed_by"] is None:
            ...
            await self.send_log(guild, "✋ Ticket übernommen", discord.Color.orange(), [("Ticket", channel.mention), ("Von", interaction.user.mention)])
        else:
    ...
    async def change_status(self, interaction: discord.Interaction, new_status: str, view: TicketControlView):
        ...
        status_names = {"ACTIVE": "🟢 Aktiv", "WAITING_USER": "🟡 Wartet auf User", "WAITING_TEAM": "🔴 Wartet auf Team", "PAUSED": "⏸️ Pausiert"}
        ...
        if config["dm_notifications"] and user:
            await self.send_dm(user, "🔄 Ticket Status aktualisiert", f"Der Status deines Tickets auf **{guild.name}** wurde aktualisiert zu:\n**{status_names.get(new_status)}**")
        await self.send_log(guild, "🔄 Status geändert", discord.Color.blurple(), [("Ticket", channel.mention), ("Neuer Status", status_names.get(new_status)), ("Geändert von", interaction.user.mention)])
    ...
    async def escalate_ticket(self, interaction: discord.Interaction, view: TicketControlView):
        ...
        if config["dm_notifications"] and user_obj:
            await self.send_dm(user_obj, "⚠️ Ticket eskaliert", f"Dein Ticket auf **{guild.name}** wurde an das High-Team eskaliert.\nDas High-Team kümmert sich nun um dein Anliegen.")
        await self.send_log(guild, "⚠️ Ticket eskaliert", discord.Color.red(), [("Ticket", channel.mention), ("Von", interaction.user.mention)])
    ...
    async def close_ticket(self, channel: discord.TextChannel, reason: str, user: discord.Member, interaction: discord.Interaction = None, is_auto: bool = False):
        guild = channel.guild
        config = await self.config.guild(guild).all()
        ticket_data = next((t for t in config["active_tickets"] if t["channel_id"] == channel.id), None)
        if not ticket_data: return

        # FIX: Alte Control View deaktivieren, damit man nicht doppelt klicken kann!
    ...
    async def delete_ticket_channel(self, channel: discord.TextChannel, ticket_data: dict, stars: int):
        ...
        if stars > 0 and config.get("log_channel_id"):
            ...
            if log_channel:
                ...
                try: await log_channel.send(embed=rev_embed)
                except: pass
