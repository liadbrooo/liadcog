...
if V2_AVAILABLE:
    ...
    class WhitelistButtonView(discord.ui.View):
        ...
        @discord.ui.button(label="Bewerbung starten", style=discord.ButtonStyle.primary, custom_id="fivem_wl_start_v11", emoji="📝")
        async def start_button(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            if interaction.user.id in blacklist:
                return await interaction.response.send_message(
                    "🚫 **Du befindest dich auf der Blacklist!**\n"
                    "Du wurdest von der Whitelist-Bewerbung ausgeschlossen.",
                    ephemeral=True
                )
            wl_role_id = await cog.config.guild(interaction.guild).wl_role()
            if wl_role_id:
...
class WhitelistModal(discord.ui.Modal, title="FiveM Whitelist Bewerbung"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        view = cog.build_application_view(data, status="pending", ping_mention=ping_mention)

        if not view:
            return await interaction.response.send_message(
                "❌ Components V2 nicht verfügbar. Bitte den Bot updaten.", ephemeral=True
            )
...
class RejectReasonModal(discord.ui.Modal, title="Grund für Ablehnung"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if dm_failed:
            await interaction.response.send_message("⚠️ User abgelehnt, aber DMs gesperrt.", ephemeral=True)
        else:
...
class FiveMWhitelist(commands.Cog):
    def __init__(self, bot: Red):
        ...
        if V2_AVAILABLE:
            try:
                ...
                log.error(f"[Whitelist] add_view fehlgeschlagen: {e}")

    # ---------------- HELPER ----------------

    def build_application_view(self, data: dict, status: str = "pending",
                               admin: discord.Member = None, reason: str = None,
                               ping_mention: str = ""):
        if not V2_AVAILABLE:
            return None
        ...
        header = f"**Bewerber:** <@{data['user_id']}> (`{data['user_id']}`)"
        if ping_mention:
            header += f"\n🔔 {ping_mention}"
    ...
    async def manual_add_wl(self, ctx: commands.Context, user_id: int):
        ...
        if not wl_role_id:
            return await ctx.send("❌ Keine Whitelist-Rolle hinterlegt.")

        wl_role = ctx.guild.get_role(wl_role_id)
        if not wl_role:
            return await ctx.send("❌ Whitelist-Rolle existiert nicht mehr.")
        ...
        if wl_role in member.roles:
            return await ctx.send("ℹ️ User hat die Whitelist bereits.")

        try:
            await member.add_roles(wl_role)
            await ctx.send(f"✅ {member.mention} hat nun die Rolle {wl_role.mention}.")
        except discord.Forbidden:
    ...
    async def manual_remove_wl(self, ctx: commands.Context, user_id: int):
        ...
        if wl_role not in member.roles:
            return await ctx.send("ℹ️ User hat die Whitelist gar nicht.")
    ...
    async def setup_wizard(self, ctx: commands.Context):
        ...
        await ctx.send(f"✅ WL-Rolle: {wl_role.mention}.\n\n**[3/5]** Welche Rolle soll bei neuen Bewerbungen gepingt werden? (`skip`)")
        try:
            msg = await self.bot.wait_for("message", check=check, timeout=60.0)
        except asyncio.TimeoutError:
            return await ctx.send("⏱️ Timeout.")
        if msg.content.lower() == "skip":
            ...
            await self.config.guild(ctx.guild).ping_role.set(None)
            await ctx.send("❌ Ungültig.")

        await ctx.send("**[4/5]** Weitere Rollen, die Bewerbungen bearbeiten dürfen? (`skip`)")
        try:
            msg = await self.bot.wait_for("message", check=check, timeout=60.0)
        except asyncio.TimeoutError:
            return await ctx.send("⏱️ Timeout.")
        if msg.content.lower() == "skip" or not msg.role_mentions:
            ...
            await ctx.send("✅ Extra-Rollen gesetzt.")

        await ctx.send("**[5/5]** Channel für Blacklist-Einträge? (`skip`)")
        try:
            msg = await self.bot.wait_for("message", check=check, timeout=60.0)
        except asyncio.TimeoutError:
            return await ctx.send("⏱️ Timeout.")
    ...
    async def setup_panel(self, ctx: commands.Context):
        ...
        if not V2_AVAILABLE:
            return await ctx.send("❌ Components V2 nicht verfügbar. Bitte discord.py updaten.")
